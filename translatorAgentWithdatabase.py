...
<style>
    ...
    /* --- ADDED: Style for the disabled output box to match the input --- */
...
def db_setup():
    """
    Connects to the SQLite database and creates the 'corrections' table
...
def db_save_correction(target_language, source_text, original_translation, corrected_translation):
    """
    Saves a new translation correction to the database.
...
def db_get_corrections(target_language, limit=3):
    """
    Retrieves the most recent 'limit' corrections for a specific target language.
    """
    try:
        conn = sqlite3.connect('translations.db')
        cursor = conn.cursor()
...
def translate_with_custom_model(_text, target_language, instructions):
    ...
    try:
        ...
        corrections = db_get_corrections(target_language, limit=3)
...
def translate_text(model_name, text, target_language, instructions):
    ...
    --- THIS FUNCTION IS NOW MODIFIED TO INCLUDE CORRECTIONS ---
    """
    ...
    if st.session_state.editing_translation:
        ...
        if st.session_state.text_translation_result:
            if st.button("Edit Translation", use_container_width=True):
                ...
                st.session_state.current_source_text = st.session_state.text_input
                st.session_state.text_output_edit = st.session_state.text_translation_result
                st.rerun()
